<template>
  <div class="wrapper">
    <div class="content">
      <h2 class="title">Candi Prambanan</h2>
      <img src="@/assets/Candi Prambanan.jpg" alt="Candi Prambanan" class="main-image" />
      ...
      <section v-if="groupedData['Fungsi Sekarang']" class="info-section">
       ...
       <!--Arca, Relief, dan Candi Pendamping -->
    <section class="info-section">
    <h3>Terdapat Arca dan Relief pada Candi Prambanan:</h3>
      <ul>
        <li><router-link to="/arca-relief-prambanan" class="custom-link">Arca & Relief</router-link></li>
      </ul>
    </section>


      <section class="info-section">
  <h3>Berikut candi-candi pada Candi Prambanan:</h3>
  <ul>
    <li><router-link to="/candi-Utama" class="custom-link">Candi Utama</router-link></li>
    <li><router-link to="/candi-Pendamping" class="custom-link">Candi Pendamping</router-link></li>
  </ul>
</section>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    fetchDbpediaDescription() {
      ...
      const query = `
        SELECT ?abstract WHERE {
          <http://dbpedia.org/resource/Prambanan> <http://dbpedia.org/ontology/abstract> ?abstract .
    ...
    fetchDbpediaDescriptionAlternative() {
      ...
      const dbpediaUrl = 'https://dbpedia.org/data/Prambanan.json';
      ...
      .then(res => {
        const data = res.data;
        const subjectUri = 'http://dbpedia.org/resource/Prambanan';
        const description = 
  ...
  mounted() {
    ...
    const query = `
      ...
      WHERE {
        OPTIONAL {
          tb:candi_prambanan rdfs:comment ?komentar ;
        ...
        {
          tb:candi_prambanan tb:memilikiNilaiSejarah ?x .
          OPTIONAL { ?x rdfs:comment ?label }
          BIND("Nilai Sejarah" AS ?tipe)
        }
        UNION {
          tb:candi_prambanan tb:memilikiPeristiwaSejarah ?x .
          OPTIONAL { ?x rdfs:comment ?label }
          BIND("Peristiwa Sejarah" AS ?tipe)
        }
        UNION {
          tb:candi_prambanan tb:memilikiGayaArsitektur ?x .
          OPTIONAL { ?x rdfs:comment ?label }
          BIND("Gaya Arsitektur" AS ?tipe)
        }
        UNION {
          tb:candi_prambanan tb:memilikiFungsiAsli ?x .
          OPTIONAL { ?x rdfs:comment ?label }
          BIND("Fungsi Asli" AS ?tipe)
        }
        UNION {
          tb:candi_prambanan tb:memilikiFungsiSekarang ?x .
          OPTIONAL { ?x rdfs:comment ?label }
